      ...
      >
        ...
        <div className="p-8 md:p-10 flex flex-col flex-1 relative -mt-10 z-10">
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <button className="font-mono text-[10px] font-medium text-secondary hover:text-white transition-colors">[COPY]</button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-8 mb-16">
            {Object.entries(prompt.parameters).map(([key, value]) => (
              <div key={key}>
                ...
                <div className="font-sans text-[14px] text-[#e5e2e1] font-medium tracking-wide">{value}</div>
              </div>
            ))}
          </div>

          <div className="mt-auto grid grid-cols-2 gap-4 pt-6 border-t border-[#353534]">
            <button className="flex items-center justify-center gap-2 bg-primary text-[#3c0091] py-4 font-display text-[12px] font-bold uppercase tracking-widest hover:bg-primary/90 transition-colors rounded shadow-[0_0_15px_rgba(208,188,255,0.15)]">
